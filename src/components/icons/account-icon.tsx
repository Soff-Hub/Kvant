...
import AccountWishlistIcon from './account-wishlist';
...
const AccountIcon: React.FC<Props> = ({
  ...
  const accountMenu = [
    ...
    },
    {
      ...
      name: 'Список желаний',
      icon: <AccountWishlistIcon className="w-5 md:w-[22px] h-5 md:h-[22px]" />,
    },
  ];
  ...
  return (
    isClient && (
      <div
        className="relative min-h-[50px] py-[10px]"
      ...
      >
        ...
        {open && (
          <div className="hidden absolute top-12 right-0 lg:block  w-52">
            <AccountNav
              options={isClient && token ? accountMenu : accountMenuToken}
              lang={lang}
            />
          </div>
        )}
