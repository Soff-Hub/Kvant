...
const ProductCardV2: React.FC<ProductProps> = ({
  ...
  const { id, title, image, discount, quantity, slug, discount_price, price } =
    product ?? {};
  const { openModal } = useModalAction();
  const { t } = useTranslation(lang, 'common');
  const { width } = useWindowSize();
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col h-full overflow-hidden  relative">
        <div className="hidden text-sm  leading-6 text-gray-400 ">{price}</div>
        <Link
        ...
        <div className="space-s-2 mb-2">
          {discount_price !== Number(price) ? (
            ...
            <>
              <span className="mx-1  text-gray-400 text-opacity-70">
                {Number(price)} so'm
              </span>
            </>
          )}
