...
export default function ProductPopupPages({ lang }: { lang: string }) {
  ...
  const newPath = query.split('/').pop();

  async function handlePopupView() {
    try {
      const response = await fetch(
        `${baseURL + API_ENDPOINTS.PRODUCTS_DETAILS}/${newPath}/`,
      );
          ...
          <div className="lg:flex items-stretch justify-between gap-8">
            <div className="xl:flex  justify-center overflow-hidden">
              {!!data?.galleries?.length ? (
                <ThumbnailCarousel gallery={data?.galleries} lang={lang} />
              ) : (
                <div className="flex items-center justify-center w-auto">
                  <Image
                    src={
                      data?.galleries?.[0]?.image ?? productGalleryPlaceholder
                    }
                    alt={data?.title}
                    width={650}
                    height={590}
